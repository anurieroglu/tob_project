...
def average_precision_from_probs(probs: torch.Tensor, target: torch.Tensor, n_thresh: int = 101) -> float:
    ...
    return float(np.trapezoid(precisions, recalls))  # deprecation-safe
...
class ResNet50_UNet(nn.Module):
    ...
    Decoder: klasik UNet (P4->P3->P2->P1), proj_ch ile kanal eşitleme.
    Çıkış: (B, num_classes, H/4, W/4) — loss/metric sırasında GT boyutuna upsample edilir.
    """
    def __init__(self, num_classes=2, proj_ch=256, pretrained=False):
        ...
        self.enc1 = backbone.layer1  # 1/4, 256
        self.enc2 = backbone.layer2  # 1/8, 512
        self.enc3 = backbone.layer3  # 1/16, 1024
        self.enc4 = backbone.layer4  # 1/32, 2048

        C = proj_ch
        self.p1 = nn.Conv2d(256,  C, 1, bias=False)
        self.p2 = nn.Conv2d(512,  C, 1, bias=False)
        self.p3 = nn.Conv2d(1024, C, 1, bias=False)
        self.p4 = nn.Conv2d(2048, C, 1, bias=False)

        # Decoder (UNet)
        self.d3 = ConvBlock(C + C, C)  # [p3, up(p4)]
        self.d2 = ConvBlock(C + C, C)  # [p2, up(d3)]
        self.d1 = ConvBlock(C + C, C)  # [p1, up(d2)]
    ...
    def forward(self, x):
        ...
        p1 = self.p1(e1); p2 = self.p2(e2); p3 = self.p3(e3); p4 = self.p4(e4)

        d3 = self.d3(torch.cat([p3, _ups(p4, p3.shape[-2:])], dim=1))
        d2 = self.d2(torch.cat([p2, _ups(d3, p2.shape[-2:])], dim=1))
        d1 = self.d1(torch.cat([p1, _ups(d2, p1.shape[-2:])], dim=1))

        logits = self.classifier(d1)    # (B, num_classes, H/4, W/4)
        return logits
...
def save_checkpoint(model, path: Path):
    ...
    cfg = {"arch":"ResNet50_UNet","num_classes":2,"proj_ch":256}
...
def train_and_eval(args):
    ...
    model = ResNet50_UNet(num_classes=2, proj_ch=256, pretrained=False).to(device)
...
# -----------------------
def parse_args():
    p = argparse.ArgumentParser(description="ResNet50 + UNet segmentation (train/eval).")
    p.add_argument("--images", type=str, required=True)
    p.add_argument("--masks",  type=str, required=True)
    p.add_argument("--output_dir", type=str, default="outputs_r50_unet")
